...
import { RobotAgentProcess } from "@robocloud/robot-agent/src/agent.js";
...
describe("RoboCloud E2E Integration", () => {
  ...
  it("should list the connected robot", async () => {
  ...
  it("should create a session and receive telemetry via WebSocket", async () => {
    ...
    const deleteRes = await fetch(`${serverUrl}/sessions/${session.id}`, {
      method: "DELETE",
    });
    expect(deleteRes.status).toBe(200);
    const ended = await deleteRes.json() as any;
    ...
    for (let i = 0; i < 10; i++) {
      const cmd = create(CommandSchema, {
        command: {
          case: "jointPosition",
          value: {
            positions: { shoulder_pan: i * 0.1, shoulder_lift: 0, elbow: 0, wrist_1: 0, wrist_2: 0, wrist_3: 0 },
          },
        },
      });
